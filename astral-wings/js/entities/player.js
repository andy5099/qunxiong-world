...
export const player = (save) => {
  ...
  x: 180, y: 555, r: 6,
  hp: Math.floor((100 + save.level * 5 + bonus + (save.star - 1) * 8) * (1 + vitality)), maxHp: Math.floor((100 + save.level * 5 + bonus + (save.star - 1) * 8) * (1 + vitality)),
  shield: Math.floor((50 + save.level * 3 + Math.floor(bonus * 0.45) + (save.star - 1) * 4) * (1 + vitality)), maxShield: Math.floor((50 + save.level * 3 + Math.floor(bonus * 0.45) + (save.star - 1) * 4) * (1 + vitality)),
  atk: Math.floor((10 + save.level * 2 + Math.floor(bonus * 0.4) + (save.star - 1) * 2) * (1 + attack + fusionGrowth)), energy: 0, inv: 0, fire: 0,
