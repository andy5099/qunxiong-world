...
export function render(ctx, state) {
  const { p, enemies, bullets, pickups, particles, boss, stars } = state;
  ...
  enemies.forEach((entry) => triangle(entry, entry.color));
  if (boss) {
    ctx.save();
  ...
  ctx.globalAlpha = 1;
  if (state.bossEntrance > 0) { ctx.fillStyle = '#ff5f7a'; ctx.font = 'bold 19px system-ui'; ctx.textAlign = 'center'; ctx.fillText(state.message, 180, 300); }
  ctx.restore();
}
