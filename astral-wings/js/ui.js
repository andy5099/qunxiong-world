...
export const equipmentView = state => {
  ...
  const synthesis = qualities.map((quality, index) => {
    const available = owned.filter(item => { const source = equipmentTemplates.find(template => template.id === item.id); return source?.quality === quality && !item.locked && !Object.values(state.equipped).includes(item.id); }).length;
    return `${button(`${quality}裝備 ${available}/3 → ${qualities[index + 1]} 合成`, `synth:${quality}`)}`;
  }).join('');
  return `<div class="shell panel menu"><h2>裝備與強化</h2><p class="note">Boss 通關保證取得戰利品。強化 100% 成功；已裝備或鎖定物不可分解／合成。</p>${button('消耗 25 材料合成新裝備','craft')}<h3>低階升階</h3>${synthesis}<div class="equip-grid">${cards.join('')}</div>${button('返回主選單','home')}</div>`;
};
