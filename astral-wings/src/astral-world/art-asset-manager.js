...
export function applyUiArtAssets(root=globalThis.document){
  if(!root)return 0;const bindings=[['ui.battleHud','.combat-readout'],['ui.skillFrame','.skill-card'],['ui.equipmentFrame','.slot'],['ui.bossBar','.boss-button'],['ui.button','.action'],['ui.primaryButton','.boss-button'],['ui.panelTitle','.page-head']];let applied=0;
  for(const [id,selector] of bindings){const image=getArtAsset(id);if(!image)continue;root.querySelectorAll(selector).forEach(node=>{node.style.setProperty('--aw-art-image',`url("${image.src}")`);node.classList.add('has-art-image');applied+=1;});}return applied;
}
