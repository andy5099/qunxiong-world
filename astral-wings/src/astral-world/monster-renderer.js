...
export function drawMonster(ctx, enemy, { time = 0, x = 278, y = 267, attackIn = 1, powerSave = false } = {}) {
  ...
  if(image&&definition){drawSpriteAnimation(ctx,image,{...definition,elapsed:time,x:x+knock,y:y+bob,scale:(enemy.boss?.42:.38)*size,loop:definition.loop!==false,powerSave});return{x:x+knock,y:y+bob,scale:size,rage,alpha:dying?deathProgress:spawnProgress};}
