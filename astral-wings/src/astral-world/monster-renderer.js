...
export function drawMonster(ctx, enemy, { time = 0, x = 278, y = 267, attackIn = 1, powerSave = false } = {}) {
  ...
  if(image&&definition){const animation=enemy.boss?(dying?'death':state.hurt?'hurt':rage?'rage':state.attack?'attack':'idle'):(dying?'death':state.hurt?'hurt':enemy.spawnIn>0?'spawn':state.attack?'attack':'idle'),range=definition.animations?.[animation]||[0,definition.frameCount];drawSpriteAnimation(ctx,image,{...definition,frameOffset:range[0],frameCount:range[1],elapsed:time,x:x+knock,y:y+bob,scale:(enemy.boss?.42:.38)*size,loop:animation==='idle'||animation==='rage',powerSave});return{x:x+knock,y:y+bob,scale:size,rage,alpha:dying?deathProgress:spawnProgress};}
