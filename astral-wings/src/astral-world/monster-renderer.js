...
export function drawMonster(ctx, enemy, { time = 0, x = 278, y = 267, attackIn = 1, powerSave = false } = {}) {
  ...
  const id=enemy.boss&&enemy.visualType==='crownBeast'?'boss.region01.crownedBeast':({slime:'monster.region01.starSlime',rabbit:'monster.region01.moonRabbit',beetle:'monster.region01.starBeetle'}[enemy.visualType]);const image=id&&getArtAsset(id),definition=id&&getArtAssetDefinition(id);
  if(image&&definition){const animation=enemy.boss?(dying?'death':state.hurt?'hurt':rage?'rage':state.attack?'attack':'idle'):(dying?'death':state.hurt?'hurt':enemy.spawnIn>0?'spawn':state.attack?'attack':'idle'),range=definition.animations?.[animation]||[0,definition.frameCount],loop=animation==='idle'||animation==='rage',elapsed=loop?time:animation==='death'?Math.max(0,(enemy.deathDuration||.46)-(enemy.deathIn||0)):animation==='spawn'?Math.max(0,(enemy.spawnDuration||.36)-(enemy.spawnIn||0)):Math.max(0,.3-attackIn);drawSpriteAnimation(ctx,image,{...definition,frameOffset:range[0],frameCount:range[1],elapsed,x:x+knock,y:y+bob,scale:(enemy.boss?.42:.38)*size,loop,powerSave});return{x:x+knock,y:y+bob,scale:size,rage,alpha:dying?deathProgress:spawnProgress};}
