...
export function drawPet(ctx, pet, { time=0, action='idle', actionIn=0, returnIn=0, summonIn=0, powerSave=false } = {}, x=142, y=300) {
  if (!pet) return;
  const attacking=action==='attack'; const returning=action==='return'; const celebrating=action==='celebrate';
  ...
  const state={time,attack:attacking,hurt:action==='hurt',powerSave}; const drawer=PET_DRAWERS[pet.visualType]||drawFallbackPet;
