import { PET_SOURCE_KINDS, PET_STAR_BALANCE, PET_VISUALS } from './data.js';
...
export function getPetEffectiveAttack(pet = {}) { return round((pet.baseAttack ?? pet.attack ?? 0) * getPetStarMultiplier(pet.stars)); }
export function getPetEffectiveHpBonus(pet = {}) { return round((pet.baseHpBonus ?? pet.hpBonus ?? 0) * getPetStarMultiplier(pet.stars)); }
...
export function normalizePet(pet = {}) {
  ...
  const multiplier = getPetStarMultiplier(stars);
  ...
  const normalized = { ...pet, id:pet.id || speciesId, speciesId, sourceKind, captureTier, visualType:pet.visualType || visual.visualType, species:pet.species || visual.species, palette:pet.palette || visual.palette, evolutionStage:Math.max(1, Number(pet.evolutionStage) || 1), stars, baseAttack, baseHpBonus };
...
export function migratePets(pets = [], fragments = {}, activePetId = null) {
  ...
  for (const [speciesId, group] of groups) {
    group.sort((a, b) => b.stars - a.stars || b.level - a.level || b.baseAttack - a.baseAttack);
