...
export class AstralUI {
  ...
  item(item) { const equipped=this.state.equipped[item.slot]?.id===item.id; return `<article class="item-card quality-${item.quality}"><button class="icon" data-action="inspect" data-id="${item.id}">${iconFor(item.slot)}</button><div><h3>${item.name} +${item.enhance||0}</h3><p>${item.label} · Lv.${item.level} · 戰力 ${equipmentPower(item)} · 詞綴 ${(item.affixes||[]).length}</p><p>${Object.entries(item.stats).map(([k,v])=>`${k}+${Math.round(v)}`).join('　')}</p></div><div class="tiny-actions"><button data-action="equip" data-id="${item.id}">${equipped?'已穿戴':'裝備'}</button><button data-action="enhance" data-id="${item.id}">強化</button><button data-action="inspect" data-id="${item.id}">詳情</button><button data-action="lock" data-id="${item.id}">${item.locked?'解鎖':'鎖定'}</button><button data-action="sell" data-id="${item.id}">出售</button></div></article>`; }
  inspect(id) { const item=this.state.inventory.find(entry=>entry.id===id); if(!item)return; const current=this.state.equipped[item.slot]; const comparison=compareEquipment(item,current?.id===item.id?null:current); const sign=value=>value>0?`<em class="up">▲ +${Math.round(value*100)/100}</em>`:value<0?`<em class="down">▼ ${Math.round(value*100)/100}</em>`:'<em>— 不變</em>'; const affixes=(item.affixes||[]).map(a=>`<li>${a.label} +${a.mode==='percent'?(a.value*100).toFixed(1)+'%':a.value}</li>`).join('')||'<li>此裝備沒有隨機詞綴</li>'; const cost=enhanceCost(item),chance=Math.round(enhanceChance(item)*100); this.openModal(`${item.name} +${item.enhance||0}`,`<b>${item.label} · ${SLOTS[item.slot].label} · Lv.${item.level}</b><br>戰力 ${equipmentPower(item)} ${comparison.power>0?'· 推薦裝備':''}<br><br><b>隨機詞綴</b><ul>${affixes}</ul><b>與目前裝備比較</b><br>戰力 ${sign(comparison.power)}<br>攻擊 ${sign(comparison.stats.attack)}　防禦 ${sign(comparison.stats.defense)}<br>生命 ${sign(comparison.stats.maxHp)}　暴擊 ${sign(comparison.stats.crit)}<br><br>下次強化：${cost} 金幣 · 成功率 ${chance}%`, `<button data-action="enhance" data-id="${item.id}">強化</button><button data-action="modalClose">關閉</button>`); }
