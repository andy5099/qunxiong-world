...
3. 切回【劇情】按【生成開場】，直接進入具體地點、具名 NPC 正在行動的事件。每幕固定三個動態方向，點 ①②③ 將實際意圖送給 AI，再生成下一幕。自由文字放在折疊的【✎ 自訂行動】。
...
- `ai-schema.js`：必填欄位、固定 3 個不重複 label／intent（讀取舊存檔允許 0–3 個）、有界文字／數值、成年人物、ID allowlist；拒絕任意效果程式與未知欄位。
...
每幕提示要求有新結果，NPC 依人格、目標、關係與記憶主動行動；世界規則只留在上下文。重複上一幕文字、整組重用選項或連續同地聊天仍無結構化進展會拒絕提交。這些檢查無法保證自然語言的趣味或語意差異，仍需真實模型驗收。舊版 0–2 選項場景可原樣讀取，按接續生成三選項即可繼續。

## 記憶與世界連續性

每世界獨立保存場景、最近 12 幕完整文字、世界時間、AI 人物、地點、物品／勢力／秘密／事件／能力、任務與伏筆。AI NPC 有 Character Card，可在角色頁查看、相處並保存共同記憶。

普通交談只保留在近期／角色記憶。event 類需標 importance 為 relationship、major-choice、world-change 或 ability 才加入永久重要事實；routine 永不加入。承諾、關係、能力與伏筆等重要種類仍保留，既有 ledger 不刪除。
