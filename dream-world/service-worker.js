const CACHE='dream-world-v0.2.3-diagnostics2';
...
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(FILES.map(f=>new Request(new URL(f,BASE).href,{cache:'reload'})))).then(()=>self.skipWaiting())));
