import { validateAISave } from './ai-save.js';
...
function record(v,check) {
  if(!plain(v) || Object.keys(v).length>1000)throw new Error('存檔資料表格式不正確');
...
export function validateWorldState(raw, options={}) {
  ...
  s.ai=validateAISave(raw.ai,options.checkpoint?null:r=>validateWorldState(r,{checkpoint:true}));
  if(s.ai.checkpoint && s.ai.checkpoint.state.worldId!==world.id)throw new Error('AI 檢查點世界不符');
  if(typeof raw.sceneId!=='string' || !Object.hasOwn(world.events,raw.sceneId) || !(world.locations.some(l=>l.id===raw.location)||Object.hasOwn(s.ai.locations,raw.location)))throw new Error('存檔場景不存在');
  ...
  s.aiCharacters=list(raw.aiCharacters??[],100,c=>{
    if(!/^ai-[a-zA-Z0-9_-]{1,57}$/.test(c.id)||c.adult!==true)throw new Error('AI 角色格式錯誤');
    const p=c.personas?.find(p=>p.worldId===world.id);if(!p)throw new Error('AI 角色缺少身份');
    const card=createAdultCharacter({...c,...p},world.id,c.id);
    for(const key of ['boundaries','dislikes','weakness','secret'])card[key]=text(c[key],400);
    return card;
  });
  const ids=[...world.characters,...s.customCharacters.map(c=>c.id),...s.aiCharacters.map(c=>c.id)];
  ...
  s.memory={short:list(raw.memory.short,8,memory),long:list(raw.memory.long,40,memory),recent:list(raw.memory.recent,12,m=>({turn:number(m.turn,0,1e7),choice:text(m.choice,200),text:text(m.text,2000)})),relationshipSummary:text(raw.memory.relationshipSummary,20000),worldSummary:text(raw.memory.worldSummary,10000)};
  ...
  if(s.ai.scene && s.ai.scene.participants.some(id=>!ids.includes(id)))throw new Error('AI 場景人物遺失');
  if(raw.ai===undefined)for(const [i,m] of s.memory.long.entries())s.ai.facts['legacy-'+i]={id:'legacy-'+i,text:m.text,character:m.character,kind:'event'};
  return s;
}
export function validateSave(raw) {
  ...
  const worlds=list(raw.worlds,12,r=>validateWorldState(r)),ids=worlds.map(w=>w.worldId);
