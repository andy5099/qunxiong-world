...
import { buyItem, buyCaptureItem } from './shop.js';
...
import { spawn,win,capture,deploy,star,currentMap,gearAction } from './hunting.js';
...
function huntOnce(){const p=state.player,auto=p.hunting.auto;if(auto.heal&&p.hp<p.maxHp*.45&&p.inventory.herb){useHerb(p);}if(p.hp/p.maxHp*100<=auto.retreat){stopHunt();state.notice='生命低於回城門檻，自動狩獵已停止。';state.noticeType='bad';return;}const e=state.huntEnemy||spawn(p);state.huntEnemy=e;if(e.kind==='boss'&&state.autoHunting&&auto.stopOnBoss&&!auto.autoBoss){stopHunt();state.notice=`${e.name}出現，自動狩獵已暫停。`;state.noticeType='good';return;}if(e.kind==='boss'&&e.hp/e.maxHp<=.2){e.catchable=true;if(state.autoHunting&&auto.autoCapture){const r=capture(p,e,auto.captureItem);setNotice(r);if(r.ok||e.escaped)state.huntEnemy=null;}else{state.notice=`${e.name}生命低於 20%，可使用收服符。`;state.noticeType='good';}return;}const boss=p.bossPartners?.find(x=>x.deployed);const hit=Math.max(1,(p.baseAttack+(boss?boss.attack:0))-e.defense+Math.floor(Math.random()*4));e.hp-=hit;if(e.hp<=0){const r=win(p,e);state.huntEnemy=null;state.notice=`擊敗${e.name}，獲得 EXP ${e.exp}、金錢 ${e.gold}${r.g?`，掉落【${r.g.quality}】${r.g.name}`:''}。`;state.noticeType=r.g&&['稀有','史詩','傳說','神器'].includes(r.g.quality)?'good':'';return;}p.hp=Math.max(0,p.hp-Math.max(1,e.attack-p.baseDefense));if(!p.hp){p.hp=Math.ceil(p.maxHp*.35);state.huntEnemy=null;state.notice='狩獵失敗，已返回安全地點。';state.noticeType='bad';}}
...
app.addEventListener('click',event=>{const target=event.target.closest('button[data-action]');if(!target||target.disabled)return;const action=target.dataset.action;
 ...
 else if(action==='shop'||action==='inventory'||action==='character'||action==='maps'||action==='hunting'||action==='bosses'||action==='settings'){state.screen=action;} else if(action==='tavern'){state.screen='tavern';}
 ...
 else if(action.startsWith('capture:')){const result=capture(state.player,state.huntEnemy,action.slice(8));setNotice(result);if(result.ok||state.huntEnemy?.escaped)state.huntEnemy=null;}
 else if(action.startsWith('gear:')){const [,kind,id]=action.split(':');setNotice(gearAction(state.player,kind,id));}
 else if(action.startsWith('auto:')){const key=action.slice(5);state.player.hunting.auto[key]=!state.player.hunting.auto[key];setNotice({ok:true,message:`自動設定「${key}」已切換。`});}
 else if(action.startsWith('speed:')){state.player.hunting.auto.speed=Number(action.slice(6));setNotice({ok:true,message:'戰鬥速度已更新。'});if(state.autoHunting)scheduleHunt();}
 ...
 else if(action.startsWith('buy-capture:'))setNotice(buyCaptureItem(state.player,action.slice(12)));
