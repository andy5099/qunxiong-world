...
app.addEventListener('click',event=>{const target=event.target.closest('button[data-action]');if(!target||target.disabled)return;const action=target.dataset.action;
 ...
 else if(action==='load'){const saved=loadGame();if(saved){state.player=normalizePlayer(saved);state.tavernRoster=createTavernRoster();state.screen='village';state.notice=`歡迎回來，${state.player.name}。`;state.noticeType='good';}}
 ...
 else if(action==='shop'||action==='inventory'||action==='character'){state.screen=action;} else if(action==='tavern'){state.screen='tavern';}
