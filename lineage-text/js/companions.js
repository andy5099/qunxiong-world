import{PET_CLASS_LIMIT,petExpNeed,totalControlUsed,remainingControl}from'./balance.js?v=26';import{PET_TYPES,SUMMON_TYPES,PET_EVOLUTIONS,controlCapacity}from'./systems.js?v=26';
export function companionHits(p){let hits=p.pets.filter(x=>p.activePets.includes(x.uid)&&x.alive).map(pet=>{let pd=PET_TYPES.find(x=>x.id===pet.type);return{name:pet.name,damage:Math.floor((pd?.atk||0)*(pet.evolutionMultiplier||1)*(1+pet.level/20))}}),sum=p.summons[0],sd=sum?SUMMON_TYPES.find(x=>x.id===sum.type):null;if(sd)hits.push({name:sd.name,damage:sd.atk});return hits}export function companionAttack(p){return companionHits(p).reduce((n,x)=>n+x.damage,0)}
...
export function togglePet(p,uid){let pet=p.pets.find(x=>x.uid===uid);if(!pet||!pet.alive)return false;let i=p.activePets.indexOf(uid);if(i>=0){p.activePets.splice(i,1);return true}if(p.activePets.length>=PET_CLASS_LIMIT[p.cls])return false;p.activePets.push(uid);return true}
