...
let mage=make('法師','CHA驗收').player;mage.level=72;mage.learnedSkills=['召喚術','高階召喚術'];for(const item of CLASSIC_STAT_GEAR.filter(x=>CHARM_SET.pieceIds.includes(x.id)||['cha-amulet','summon-ring'].includes(x.id))){addItem(mage,item);ok(equip(mage,mage.bag.at(-1).uid),`無法裝備 ${item.name}`)}eq(derived(mage).stats.cha,28,'終局 CHA 應可達 28');for(const rank of['召喚術','高階召喚術']){let unit=SUMMON_TYPES.filter(x=>x.rank===rank).at(-1),status=summonStatus(mage,unit.id);ok(status.ok,`${unit.name} 不可達：${status.reasons}`);ok(summon(mage,unit.id),`${unit.name} 召喚失敗`)}ok(SUMMON_TYPES.every(x=>summonStatus(mage,x.id).currentCha>=x.requiredCha),'存在不可達 CHA 召喚');ok(!SUMMON_TYPES.some(x=>x.boss||/安塔瑞斯|法利昂|林德拜爾|巴拉卡斯|死亡騎士|惡魔|巫妖|鐮刀死神/.test(x.name)),'不應有自創 Boss 召喚');
