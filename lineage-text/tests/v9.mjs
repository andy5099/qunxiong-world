...
let sorted=[...totals].sort((a,b)=>a-b),avg=totals.reduce((a,b)=>a+b,0)/totals.length;ok(avg>=3&&avg<=8,`Lv27 平均 ${avg}`);ok(zeros/500<=.05,`零本率 ${zeros/500}`);ok(sorted[250]>=3&&sorted[250]<=8,`中位數 ${sorted[250]}`);
