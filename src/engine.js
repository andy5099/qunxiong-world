import { AREAS, BOSS_PITY_LIMIT, BOSS_RECOMMENDED_POWER, DUNGEON, YELLOW_DUNGEON, ENEMIES, EXP_TO_LEVEL, INN_COST, ITEMS, SLOT_NAMES, STAT_NAMES, createBlackwindLeader } from './data.js?v=v023-prototype-v21';
import { applyLeaderRarity, createRarityBoss, getBossRarity, getCaptureRate, rollBossRarity, rollTalismanDrops, TALISMANS } from './boss-progression.js?v=v023-prototype-v21';
import { DIVINE_TALISMANS, getBlackwindResonance, getBossGearInfo, rollDivineTalismanDrops } from './boss-gear-system.js?v=v023-prototype-v21';
import { WORLD_BOSS, WORLD_BOSSES, addWorldBossToRoster, createWorldBossEnemy, getWorldBossMasteryState, getWorldBossRecordState, getWorldBossResonance, getWorldBossState } from './world-boss-system.js?v=v023-prototype-v21';
import { awardWorldBossMastery, getMasteryProfile, recordBlackwindCapture, recordBlackwindDefeat, recordBlackwindEncounter, recordItemDrop, recordMaterials } from './boss-codex-system.js?v=v023-prototype-v21';
import { getBreakthroughProfile } from './world-boss-breakthrough.js?v=v023-prototype-v21';
import { CHAPTER2_BOSSES, getChapter2Resonance, recordChapter2Boss, recruitChapter2Boss, spareChapter2Boss } from './chapter2-system.js?v=v023-prototype-v21';
import { ensureFormation, preparePuzzleTurn, settleFormationPuzzle, startFormationPuzzle } from './formation-puzzle.js?v=v023-prototype-v21';
import { ensureMarbleBattle, getCurrentMarble, getFormationRole, getFormationTier, getMarbleSkill, getMarbleUltimate, getUltimateEnergy, hitMultiplier } from './marble-battle.js?v=v023-prototype-v21';
...
export function resolveMarbleEvent(state,event,rng=Math.random){
  ...
  if(event.type==='wall'){marble.shot.wallBounces++;if(marble.prototypeV2&&marble.skills?.[memberIndex]?.armed&&member.id==='blackwind-lord')marble.effects.push({type:'wind-wall',characterId:member.id,x:event.x||marble.entities[memberIndex]?.x||180,y:event.y||marble.entities[memberIndex]?.y||220,life:.45});return event;}
  ...
  if(marble.phase==='pinball'){
    marble.comboTime=3;
    const formationTier=getFormationTier(hit);if(formationTier>marble.formationReady){marble.formationReady=formationTier;if(marble.prototypeV2){marble.comboMilestone=formationTier;marble.effects.push({type:'milestone',x:180,y:150,text:`${[0,10,20,35,50][formationTier]} HIT！`,life:1});}}
    const formation=marble.formationActive,formationRole=formation?.role,formationPower=formation?.tier||0;if(formation){formation.hits++;if(formationRole==='blackwind'&&formation.hits===1)multiplier*=1.22+formationPower*.15;if(formationRole==='blackwind'&&formationPower===4)multiplier*=1.12;if(formationRole==='vanguard'){multiplier*=1+formationPower*.1;if(event.weak)multiplier*=1+formationPower*.06;}if(formationRole==='world')multiplier*=1+formationPower*.08;if(formationRole==='breaker'&&formationPower>=3&&formation.hits===1)target.marbleDefenseDown=1;if(formationRole==='support'&&formation.hits===1){for(const ally of state.party.filter(alive)){const allyStats=getFinalStats(state,ally);ally.hp=Math.min(allyStats.maxHp,ally.hp+Math.round(allyStats.maxHp*.03*formationPower));}}}
    let breakGain=event.weak?(marble.prototypeV2?42:34):0;if(marble.prototypeV2&&marble.skills?.[memberIndex]?.armed&&member.id==='zhang-fei')breakGain+=36;if(formationRole==='breaker')breakGain+=8*formationPower+(event.weak?10*formationPower:0);if(event.weak&&marble.breakImmunity<=0){marble.breakGauge=Math.min(100,(marble.breakGauge||0)+breakGain);if(marble.breakGauge>=100&&marble.breakTime<=0){marble.breakTime=3.5;marble.lastProgressAt=Date.now();marble.effects.push({type:'break',x:180,y:120,text:'BREAK！',life:1.2});}}
    if(marble.breakTime>0)multiplier*=1.6;
    const slot=marble.skills?.[memberIndex];
    if(slot?.armed){multiplier*=member.id==='yellow-commander'&&target.hp/target.maxHp<.35?2.65:2;slot.armed=false;slot.energy=0;marble.hitStop=member.id==='guan-yu' ? .15 : member.id==='zhang-fei' ? .13 : .09;marble.shakeTime=member.id==='zhang-fei' ? .3 : .18;marble.flashTime=.12;marble.effects.push({type:'skill-impact',characterId:member.id,x:marble.boss.x,y:marble.boss.y,weak:event.weak,life:.75});}
    ...
    marble.ultimateGauge=Math.min(100,(marble.ultimateGauge||0)+5+(event.weak?4:0));if(marble.ultimateGauge>=100&&marble.breakTime>0&&!marble.ultimateQueued&&!marble.ultimateArmedAuto){marble.ultimateQueued=true;marble.skillQueue.push({type:'ultimate',index:0,priority:3});}if(marble.ultimateArmedAuto){multiplier*=2.35;marble.ultimateGauge=0;marble.ultimateArmedAuto=false;marble.hitStop=.18;marble.shakeTime=.42;marble.flashTime=.18;marble.effects.push({type:'ultimate-impact',characterId:member.id,x:marble.boss.x,y:marble.boss.y,weak:event.weak,life:1.1});}
  }
  ...
  marble.effects.push({type:'damage',characterId:member.id,x:marble.boss.x,y:marble.boss.y-12,text:`-${damage}`,weak:event.weak,skill:marble.skillCinematic?.characterId===member.id,ultimate:Boolean(marble.shot.ultimate),life:1});
...
export function updateFlipperSystems(state,dt,rng=Math.random){
  ...
  dt=Math.min(.05,Math.max(0,Number(dt)||0));marble.schedulerCooldown=Math.max(0,(marble.schedulerCooldown||0)-dt);marble.bossAttackIn=Math.max(0,(marble.bossAttackIn||7)-dt);marble.fieldPulseIn=Math.max(0,(marble.fieldPulseIn||5)-dt);
  ...
  if(marble.schedulerCooldown<=0&&marble.skillQueue.length){marble.skillQueue.sort((a,b)=>a.priority-b.priority);const action=marble.skillQueue.shift(),unit=state.party[action.index];if(action.type==='main'){const slot=marble.skills[action.index];if(slot){slot.queued=false;const skill=getMarbleSkill(unit);marble.skillCinematic={characterId:unit.id,index:action.index,name:skill.name,ultimate:false,elapsed:0,duration:unit.id==='liu-bei'?1.15:1.05};marble.hitStop=.12;if(marble.prototypeV2&&unit.id==='liu-bei'){slot.energy=0;for(const ally of state.party.filter(alive)){const stats=getFinalStats(state,ally),heal=Math.round(stats.maxHp*.08);ally.hp=Math.min(stats.maxHp,ally.hp+heal);marble.effects.push({type:'heal',characterId:unit.id,x:marble.entities[state.party.indexOf(ally)]?.x||180,y:marble.entities[state.party.indexOf(ally)]?.y||300,text:`+${heal}`,life:1});}marble.effects.push({type:'team-buff',characterId:unit.id,x:180,y:260,life:1.1});appendLog(state,'劉備自動發動【仁德天下】，全隊恢復兵力！','epic');}else{slot.armed=true;const entity=marble.entities[action.index];if(entity){entity.dashCooldown=0;if(unit.id==='guan-yu'){const dx=marble.boss.x-entity.x,dy=marble.boss.y-entity.y,d=Math.max(1,Math.hypot(dx,dy));entity.vx=dx/d*670;entity.vy=dy/d*670;}else if(unit.id==='zhang-fei'){const dx=marble.boss.x-entity.x,dy=marble.boss.y-entity.y,d=Math.max(1,Math.hypot(dx,dy));entity.vx=dx/d*610;entity.vy=dy/d*610;}else if(unit.id==='blackwind-lord'){entity.vx=(entity.x<180?-1:1)*650;entity.vy=-320;}}appendLog(state,`${unit.name}自動發動【${skill.name}】！`,'epic');}}}else if(action.type==='ultimate'){marble.ultimateQueued=false;marble.ultimateArmedAuto=true;const lead=state.party[0];marble.skillCinematic={characterId:lead?.id||'hero',index:0,name:getMarbleUltimate(lead).name,ultimate:true,elapsed:0,duration:2};marble.hitStop=.15;appendLog(state,'全軍奧義進入連鎖！','epic');}else{const support=marble.supports.find(item=>item?.index===action.index);if(support){support.queued=false;support.energy=0;support.ready=false;let name='援護';if(unit.id==='liu-bei'){name='仁德支援';for(const ally of state.party.filter(alive)){const stats=getFinalStats(state,ally);ally.hp=Math.min(stats.maxHp,ally.hp+Math.round(stats.maxHp*.12));}marble.skills.forEach(slot=>{if(slot)slot.energy=Math.min(100,slot.energy+12);});}else if(unit.id==='yellow-captain'){name='鐵壁援護';marble.supportGuard=1;}else{const names={'zhang-bao':'黃天雷援','yellow-commander':'破軍追擊','crimson-tiger':'赤焰追擊','nether-thunder-beast':'九幽雷援'};name=names[unit.id]||'戰意援護';const raw=Math.max(1,Math.round(getFinalStats(state,unit).might*(marble.breakTime>0?1.4:1))),cap=battle.worldBoss?Math.max(1,Math.floor(boss.maxHp*.035)):raw,damage=Math.min(boss.hp,raw,cap);boss.hp-=damage;marble.shot.damage+=damage;}marble.effects.push({type:'support',x:180,y:150,text:name,life:1});appendLog(state,`${unit.name}發動【${name}】！`,'epic');}}marble.schedulerCooldown=.3;marble.lastProgressAt=Date.now();}
  if(marble.bossAttackIn<=0&&boss.hp>0){if(marble.breakTime>0)marble.bossAttackIn=1;else{if(marble.supportGuard){marble.supportGuard=0;appendLog(state,'鐵壁援護擋下 Boss 強襲！','rare');}else performEnemyAction(state,boss,rng);marble.bossAttackIn=battle.worldBoss?7:8;if(!state.party.some(alive)){finishDefeat(state);return true;}}}
