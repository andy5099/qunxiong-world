...
const variantText=(bossId,individual)=>{const quality=getWorldBossQuality(individual?.quality),talent=getWorldBossTalent(bossId,individual?.talentId);return `<span><b>品質：${quality.name}</b></span><span>天賦：${talent?.name||'無特殊天賦'}</span>${talent?`<small>${talent.description}</small>`:''}`;};
...
function nav(state) {
  return `<nav class="game-nav" aria-label="主要選單">
    ...
    ${button('screen:quickBattle', '快速戰鬥', state.screen === 'quickBattle' ? 'active' : '')}
...
function quickBattle(state){
  ...
  return `<section class="panel quick-battle-panel"><p class="eyebrow">V0.3.1 快速爽玩</p><h1>快速 Boss 戰</h1><p>直接使用既有正式 Boss 引擎；戰利品、招降、圖鑑與熟練度完整保留。</p>${result}${group('一般 Boss',groups.normal,'normal')}${group('世界王',groups.world,'world')}${group('隱藏 Boss',groups.hidden,'hidden')}${prep}<p class="notice">${esc(state.notice)}</p></section>`;
}
...
function party(state) {
  ...
  const bondPanel=`<section class="bond-summary"><h2>目前羈絆</h2>${activeBonds.length?`<div class="bond-chip-list">${activeBonds.map(b=>`<span class="bond-chip">【${b.name}】<small>${b.description}</small></span>`).join('')}</div>`:'<p class="empty">目前隊伍尚未形成羈絆。</p>'}</section>`;
  ...
  const candidatePanel=swapSlot==null?'':`<section class="candidate-panel"><div class="candidate-title"><h2>${current?`更換第 ${swapSlot+1} 位・${esc(current.name)}`:`加入第 ${swapSlot+1} 位`}</h2>${button('party-swap-close','關閉','mini')}</div><div class="craft-actions">${button('party-sort:power','戰力排序',sort==='power'?'active mini':'mini')}${button('party-sort:level','等級排序',sort==='level'?'active mini':'mini')}</div><div class="candidate-list">${candidates.length?candidates.map(member=>{const delta=getMemberPower(state,member)-(current?getMemberPower(state,current):0),forms=getFormableBonds(state.party,member,swapSlot);return `<article class="candidate-card ${member.worldBoss?'boss-rank-5':''}"><div><strong>${member.worldBoss?'★★★★★ 【世界王】 ':''}${esc(member.name)}</strong><span>Lv.${member.level}｜戰力 ${getMemberPower(state,member).toLocaleString()}</span><small>${CHARACTER_ROLES[member.id]||member.role||'武將'}｜相較 ${delta>=0?'+':''}${delta.toLocaleString()}</small>${forms.length?`<b class="bond-formable">可啟用：${forms.map(b=>b.name).join('、')}</b>`:''}</div>${button(`party-swap:${swapSlot}:${member.id}`,'立即換入','primary')}</article>`;}).join(''):'<p class="empty">目前沒有候補武將。</p>'}</div></section>`;
  const formation=state.party.map((member,index)=>{const gear=member?getEquippedSummary(state,member.id).map(x=>x.item?.name).filter(Boolean).join('／')||'無裝備':'',position=index<3?`主戰 ${index+1}`:`支援 ${index-2}`;return `<article class="formation-slot ${member?.worldBoss?'boss-rank-5':''}"><span>${position}</span>${member?`<strong>${member.worldBoss?'★★★★★ 【世界王】 ':''}${esc(member.name)}</strong><small>Lv.${member.level}｜戰力 ${getMemberPower(state,member).toLocaleString()}</small><small>${CHARACTER_ROLES[member.id]||member.role||'武將'}｜${esc(gear)}</small>${index===0&&member.isPlayer?'<b>固定</b>':button(`party-swap-open:${index}`,'更換','primary mini')}`:`<strong>空位</strong>${button(`party-swap-open:${index}`,'＋加入武將','primary')}`}</article>`;}).join('');
  return `<section class="panel party-panel"><p class="eyebrow">五格出戰隊伍＋候補武將</p><h1>隊伍</h1><div class="team-power">出戰隊伍 ${state.party.filter(Boolean).length} / 5・戰力 <b>${getTeamPower(state).toLocaleString()}</b></div>${bondPanel}<div class="quick-formation">${button('party-quick-best','快速編隊・最高戰力','primary',Boolean(state.battle))}</div><div class="formation-list">${formation}</div>${candidatePanel}${partyEquipmentPicker(state)}<details><summary>詳細能力與裝備</summary><div class="party-list">${state.party.map((member,index)=>memberCard(state,member,index)).join('')}</div></details>${rosterDetails()}</section>`;
}

function bonds(state){const known=new Set(state.bonds?.discovered||[]);return `<section class="panel bond-codex"><p class="eyebrow">隊伍組合收藏</p><h1>武將羈絆</h1><p>已發現 ${known.size} / ${BOND_DATA.length}</p><div class="bond-codex-grid">${BOND_DATA.map(bond=>known.has(bond.id)?`<article class="bond-card discovered"><strong>【${bond.name}】</strong><span>${bond.requiredCharacterIds.map(id=>state.party.concat(state.roster).find(u=>u?.id===id)?.name||id).join('・')}</span><b>${bond.comboName}</b><small>${bond.description}</small></article>`:`<article class="bond-card unknown"><strong>？？？</strong><span>${bond.hint}</span><small>組成特定隊伍後發現</small></article>`).join('')}</div></section>`;}
...
function worldBoss(state){
  const cards=Object.values(WORLD_BOSSES).map(profile=>{const w=getWorldBossState(state,profile.id),r=getWorldBossRecordState(state,profile.id),locked=!w.unlocked,captured=hasCapturedWorldBoss(state,profile.id),status=captured?'✓ 已收服':'未收服',talents=Object.values(WORLD_BOSS_VARIANT_CONFIG.talents[profile.id]||{}).flat();return `<article class="world-boss-card boss-rank-5 ${locked?'locked':''}"><div class="danger-stars">★★★★★ 世界王</div><h2>${locked?'？？？？？':profile.name}</h2><span class="world-capture-status ${captured?'captured':'uncaptured'}">【${locked?'未發現':status}】</span>${locked?'<p>出沒線索：完成對應章節後，祭壇將產生異動。</p>':`<div class="danger-line"><span>建議戰力 ${profile.recommendedPower.toLocaleString()}</span><span>目前戰力 ${getTeamPower(state).toLocaleString()}</span></div><dl><div><dt>遭遇／擊敗</dt><dd>${w.attempts} / ${w.defeats}</dd></div><div><dt>最高遭遇</dt><dd>${getWorldBossQuality(w.highestEncounterQuality).name}</dd></div><div><dt>最佳已收服個體</dt><dd>${captured?getWorldBossQuality(w.highestCapturedQuality).name:'尚未收服'}</dd></div><div><dt>目前個體</dt><dd>${captured?getWorldBossQuality(w.currentIndividual?.quality).name:'—'}</dd></div></dl>${captured?`<div class="world-individual">${variantText(profile.id,w.currentIndividual)}</div>`:''}<div class="talent-codex"><strong>特殊天賦圖鑑</strong>${talents.map(talent=>`<span>${w.discoveredTalents?.includes(talent.id)?`✓ ${talent.name}`:'？？？'}</span>`).join('')}</div>${button(`world-boss:challenge:${profile.id}`,captured?'再次挑戰':'挑戰世界王','boss-button')}`}</article>`;}).join('');
...
export function renderMarblePanel(state,battle){
  ...
  if(marble.phase==='pinball'){
    const activeBonds=getActiveBonds(state.party),bondHud=activeBonds.length?`<div class="pinball-bonds">${activeBonds.map(bond=>{const cooling=(marble.bonds?.cooldowns?.[bond.id]||0)>0;return `<span class="${cooling?'cooldown':'ready'}">【${bond.name}】</span>`;}).join('')}</div>`:'';
    ...
    const supports=state.party.slice(3,5).map((unit,offset)=>{if(!unit)return`<span class="pinball-support empty"><b>支援 ${offset+1}</b><small>空位</small></span>`;const slot=marble.supports?.[offset]||{energy:0,ready:false};return `<span class="pinball-support${slot.ready?' ready':''}" data-support-index="${offset+3}"><b>支援 ${offset+1}・${esc(unit.name)}</b><i><em style="width:${slot.energy}%"></em></i><small>${slot.ready?'SUPPORT READY':`支援 ${Math.round(slot.energy)}%`}</small></span>`;}).join('');
    const formation=marble.formationReady?`${marble.formationReady===4?'MAX':['','I','II','III'][marble.formationReady]} READY`:'累積 10 HIT 啟動',comboRank=marble.combo>=100?'天下無雙':marble.combo>=50?'戰意沸騰':marble.combo>=30?'無雙':marble.combo>=20?'猛攻':marble.combo>=10?'疾風':'蓄勢';
    return `<div class="marble-overlay"><section class="marble-panel pinball-panel"><header><div><p class="eyebrow">高速彈射 Boss 戰・${comboRank}</p><h2>${esc(boss.displayName||boss.name)}</h2></div><strong class="marble-hit">${marble.combo||0} HIT</strong></header>${bondHud}<div class="pinball-battle-status"><span>POWER <b data-formation-ready>${formation}</b></span><span>${marble.counterWindow>0?'COUNTER 時機！':`主將類型 ${esc(marble.leadRole)}`}</span></div><div class="marble-boss-hp"><span>Boss HP ${Math.max(0,boss.hp).toLocaleString()} / ${boss.maxHp.toLocaleString()}</span>${hpBar(boss.hp,boss.maxHp,'enemy-meter')}</div><div class="pinball-break" data-pinball-break>${marble.breakTime>0?'BREAK！傷害提升':`弱點 BREAK ${marble.breakGauge||0}%`}</div><canvas class="marble-canvas" aria-label="三武將彈射 Boss 戰場"></canvas><p class="pinball-hint">點擊左／右半場：底部 FLIP、空中 DASH；Boss 預警時 COUNTER，全隊連鎖越強。</p><div class="pinball-team">${team}</div><div class="pinball-supports">${supports}</div><button type="button" class="button danger" data-action="battle:retreat-flipper">安全撤退</button></section></div>`;
  }
...
function battle(state) {
  ...
  const finishedActions = battle.awaitingRecruit ? '' : battle.sourceContext==='quickBattle'?`${button('quick-battle:retry','再次挑戰','primary')}${button('quick-battle:switch','更換 Boss')}${button('quick-battle:home','回桃源村')}`:`${quickDrop}${button('battle:close', battle.result === 'victory' ? (battle.dungeon ? '結束本層' : state.exploration.auto ? '自動繼續中' : '繼續探索') : '返回桃源村', 'primary')}`;
...
function worldBossConfirm(state){if(!state.ui.worldBossConfirm)return '';const id=state.ui.selectedWorldBoss||'crimsonTiger',profile=WORLD_BOSSES[id]||WORLD_BOSS,captured=hasCapturedWorldBoss(state,id),variant=state.ui.worldBossCandidate||{quality:'normal',talentId:null},quality=getWorldBossQuality(variant.quality),talent=getWorldBossTalent(id,variant.talentId),record=getWorldBossState(state,id),better=captured&&compareWorldBossQuality(variant.quality,record.currentIndividual?.quality)>0,risky=getTeamPower(state)<Math.round(profile.recommendedPower*quality.statMultiplier);return `<div class="danger-overlay"><section class="danger-card boss-rank-5 ${quality.id==='legendary'?'legendary-variant':''}">${quality.id==='legendary'?'<p class="legendary-alert">【傳說個體出現！】</p>':''}<div class="danger-stars">★★★★★</div><h2>【${quality.name}・${profile.name}】</h2><span class="world-capture-status ${captured?'captured':'uncaptured'}">【${captured?'✓ 已收服':'未收服'}】</span><div class="world-individual"><strong>特殊天賦：${talent?.name||'無'}</strong><small>${talent?.description||'此個體沒有核心特殊天賦。'}</small><span>${better?'狀態：尚未收服此品質':'狀態：目前个體品質未被超越'}</span></div><p class="challenge-rating warning">☠️ 極度危險・建議戰力 ${Math.round(profile.recommendedPower*quality.statMultiplier).toLocaleString()}</p><p>${risky?'目前戰力較低，仍可硬闖。':'此個體值得挑戰。'}</p><div class="action-grid">${button('world-boss:engage',captured?'再次挑戰':'挑戰世界王','boss-button')}${button('world-boss:cancel','取消')}</div></section></div>`;}
...
export function render(state) {
  const views = { village, plain, forest, stronghold,yellowRoad,yellowCamp,yellowFortress,desolateVillage,loessSlope,thunderValley,yellowHeavenAltar, worldBoss, quickBattle, bossCodex, bonds, party, inventory, shop, settings },dungeonConfig=state.dungeon.dungeonId==='yellowTomb'?YELLOW_DUNGEON:DUNGEON;
