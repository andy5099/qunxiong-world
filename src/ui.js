import { AREAS, BOSS_PITY_LIMIT, BOSS_RECOMMENDED_POWER, CHARACTER_ROLES, DUNGEON, YELLOW_DUNGEON, ENEMIES, EXP_TO_LEVEL, INN_COST, ITEMS, QUALITY_ORDER, SLOT_NAMES } from './data.js?v=v023-flipper-chain-1';
import { compareItem, equippedCount, getEquippedSummary, getFinalStats, getMemberPower, getTeamPower, recommendMemberForItem } from './engine.js?v=v023-flipper-chain-1';
import { getBossRarity, getPromotionChance, RANK_TALISMAN, TALISMANS } from './boss-progression.js?v=v023-flipper-chain-1';
import { DIVINE_TALISMANS, getBlackwindResonance, getBossGearInfo } from './boss-gear-system.js?v=v023-flipper-chain-1';
import { WORLD_BOSS, WORLD_BOSSES, getWorldBossRecordState, getWorldBossResonance, getWorldBossState } from './world-boss-system.js?v=v023-flipper-chain-1';
import { BLACKWIND_DROPS, CODEX_MATERIALS, COLLECTION_MILESTONES, DIVINE_CODEX_MATERIALS, WORLD_BOSS_DROPS, NETHER_WORLD_BOSS_DROPS, getCodexCompletion, getHighestRank, getKnownItemName, getMasteryProfile } from './boss-codex-system.js?v=v023-flipper-chain-1';
import { getAvailableGearCount, getNextGearTier } from './gear-tier-system.js?v=v023-flipper-chain-1';
import { WORLD_BOSS_BREAKTHROUGH_COSTS, canBreakthrough } from './world-boss-breakthrough.js?v=v023-flipper-chain-1';
import { CHAPTER2_BOSSES, getChapter2Resonance } from './chapter2-system.js?v=v023-flipper-chain-1';
import { ensureFormation, FORMATION_ORBS } from './formation-puzzle.js?v=v023-flipper-chain-1';
import { ensureMarbleBattle, getMarbleSkill, getMarbleUltimate, getUltimateEnergy } from './marble-battle.js?v=v023-flipper-chain-1';
...
export function renderMarblePanel(state,battle){
  ...
  if(marble.phase==='pinball'){
    ...
    const formation=marble.formationReady?`${marble.formationReady===4?'MAX':['','I','II','III'][marble.formationReady]} READY`:'累積 10 HIT 啟動';
    const prototype=marble.prototypeV2;
    return `<div class="marble-overlay${prototype?' blackwind-prototype-v2':''}"><section class="marble-panel pinball-panel"><header><div><p class="eyebrow">${prototype?'黑風寨主 Flipper Prototype V2':'Boss 彈射戰'}</p><h2>${esc(boss.displayName||boss.name)}</h2></div><strong class="marble-hit">${marble.combo||0} HIT</strong></header><div class="pinball-battle-status"><span>${prototype?'黑風戰陣':'戰陣'} <b data-formation-ready>${formation}</b></span><span>${prototype?'單指 FLIP':'主將類型 '+esc(marble.leadRole)}</span></div><div class="marble-boss-hp"><span>Boss HP ${Math.max(0,boss.hp).toLocaleString()} / ${boss.maxHp.toLocaleString()}</span>${hpBar(boss.hp,boss.maxHp,'enemy-meter')}</div><div class="pinball-break" data-pinball-break>${marble.breakTime>0?'BREAK！傷害提升':`弱點 BREAK ${marble.breakGauge||0}%`}</div><canvas class="marble-canvas" aria-label="三武將彈射 Boss 戰場"></canvas><p class="pinball-hint">${prototype?'點擊／觸摸戰場任意位置＝雙彈板 FLIP；抓準 Launch Zone 時機可觸發 PERFECT。':'點擊左／右半場，分別啟動左右彈板；彈得越準，全隊連鎖越強。'}</p><div class="pinball-team">${team}</div>${prototype?'':`<div class="pinball-supports">${supports}</div>`}<button type="button" class="button danger" data-action="battle:retreat-flipper">安全撤退</button></section></div>`;
  }
