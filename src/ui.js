...
export function renderMarblePanel(state,battle){
  ...
  if(marble.phase==='pinball'){
    ...
    return `<div class="marble-overlay"><section class="marble-panel pinball-panel"><header><div><p class="eyebrow">Boss 彈射戰</p><h2>${esc(boss.displayName||boss.name)}</h2></div><strong class="marble-hit">${marble.combo||0} HIT</strong></header><div class="pinball-battle-status"><span>戰陣 <b data-formation-ready>${formation}</b></span><span>主將類型 ${esc(marble.leadRole)}</span></div><div class="marble-boss-hp"><span>Boss HP ${Math.max(0,boss.hp).toLocaleString()} / ${boss.maxHp.toLocaleString()}</span>${hpBar(boss.hp,boss.maxHp,'enemy-meter')}</div><div class="pinball-break" data-pinball-break>${marble.breakTime>0?'BREAK！傷害提升':`弱點 BREAK ${marble.breakGauge||0}%`}</div><canvas class="marble-canvas" aria-label="三武將彈射 Boss 戰場"></canvas><p class="pinball-hint">點擊左／右半場，分別啟動左右彈板；彈得越準，全隊連鎖越強。</p><div class="pinball-team">${team}</div><div class="pinball-supports">${supports}</div><button type="button" class="button danger" data-action="battle:retreat-flipper">安全撤退</button></section></div>`;
  }
