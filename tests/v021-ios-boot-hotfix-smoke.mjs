...
check(html.includes('src/boot.js?v=v023-flipper-chain-1'), 'classic boot guard loads before module');
...
check(sw.includes("const BUILD_VERSION = 'v023-flipper-chain-1'") && sw.includes('skipWaiting()') && sw.includes('self.clients.claim()'), 'service worker activation is current');
check(sw.includes("event.request.mode === 'navigate'") && sw.includes("fetch(request, { cache: 'no-store' })"), 'navigation remains network first');
check(sources.every(source => !/^import .*\?v=(?!v023-flipper-chain-1)/m.test(source)), 'all module imports use one build version');
