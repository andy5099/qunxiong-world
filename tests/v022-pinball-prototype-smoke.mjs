...
check(ui.includes('activateMarbleFlippers')&&ui.includes('requestAnimationFrame'),'RAF flipper renderer connected');check(ui.includes("'pointerdown'")&&ui.includes("'touchstart'"),'pointer and iPhone touch controls exist');check(view.includes('data-pinball-skill')&&view.includes('Boss 彈射戰 Prototype'),'three skill UI rendered');check(css.includes('grid-template-columns:repeat(3')&&!css.includes('overflow-x:auto'),'390px layout has no horizontal rail');check(main.includes('cleanupMarbleBattle'),'Boss exit cleanup remains centralized');
