...
equal(SAVE_VERSION,20,'save version 17');
...
const migrated=normalize({...createState('舊存檔'),version:15,bonds:undefined});equal(migrated.version,20,'old save migrates');ok(Array.isArray(migrated.bonds.discovered),'old save gains bond codex');equal(normalizeBondState({discovered:['bad']}).discovered.length,0,'unknown bond ids removed safely');
